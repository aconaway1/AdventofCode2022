...
def main():
    ...
    for move_line in move_lines:
        ...
        if match_move:
            ...
            # print(f"BEFORE SRC: {stacks[src_stack]}")
            # print(f"BEFORE DST: {stacks[dst_stack]}")
            for i in range(number_to_move):
                moved_element = stacks[src_stack][0]
                print(f"Adding {moved_element} to dst stack.")
                stacks[dst_stack].insert(0, moved_element)
                del stacks[src_stack][0]

            # print(f"AFTER SRC: {stacks[src_stack]}")
            # print(f"AFTER DST: {stacks[dst_stack]}")
            # print("-" * 20)
        else:
